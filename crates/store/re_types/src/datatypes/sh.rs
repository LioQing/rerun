...
#[repr(C)]
pub struct Sh(pub [f32; 45usize]);

::re_types_core::macros::impl_into_cow!(Sh);

impl ::re_types_core::Loggable for Sh {
    #[inline]
            ...
            };
            {
                ...
                let data0_inner_validity: Option<arrow::buffer::NullBuffer> =
                    data0_validity.as_ref().map(|validity| {
                        validity
                            .iter()
                            .map(|b| std::iter::repeat_n(b, 45usize))
                            .flatten()
                            .collect::<Vec<_>>()
                            .into()
                    });
        ...
        Self: Sized,
    {
        ...
        Ok({
            let arrow_data = arrow_data
                ...
                .with_context("rerun.datatypes.Sh#sh")?;
            if arrow_data.is_empty() {
                Vec::new()
            } else {
                let offsets = (0..)
                    .step_by(45usize)
                    .zip((45usize..).step_by(45usize).take(arrow_data.len()));
                ...
                ZipValidity::new_with_validity(offsets, arrow_data.nulls())
                    .map(|elem| {
                        elem.map(|(start, end): (usize, usize)| {
                            debug_assert!(end - start == 45usize);
        ...
        .collect::<DeserializationResult<Vec<Option<_>>>>()
        .with_context("rerun.datatypes.Sh#sh")
        .with_context("rerun.datatypes.Sh")?)
    }
        ...
        Ok({
            let slice = {
                let arrow_data = arrow_data
                    ...
                    .with_context("rerun.datatypes.Sh#sh")?;
                let arrow_data_inner = &**arrow_data.values();
                bytemuck::cast_slice::<_, [_; 45usize]>(
                    arrow_data_inner
                        .as_any()
                        .downcast_ref::<Float32Array>()
                        .ok_or_else(|| {
...
impl From<[f32; 45usize]> for Sh {
    #[inline]
    fn from(sh: [f32; 45usize]) -> Self {
        Self(sh)
    }
}

impl From<Sh> for [f32; 45usize] {
    #[inline]
    fn from(value: Sh) -> Self {
        value.0
    }
}

impl ::re_byte_size::SizeBytes for Sh {
    #[inline]
